...
if [ -z "$1" ]
then
  echo "Please input flconc server name!"
  exit -1
fi
...
echo "Shutdown flconc project, name: ${PROJECT_NAME}, path: ${PROJECT_BIN_FILE}"

# check bin file exist
if [ ! -e ${PROJECT_BIN_FILE} ]
then
  echo "Shtdown flconc eror, file: ${PROJECT_BIN_FILE} not exist, please check file"
  exit -1
fi
