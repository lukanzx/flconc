...
#include <iostream>
#include "log.h"
#include "tcoroutine_pool.h"
#include "tcoroutine_hook.h"
#include "tcoroutine.h"
#include "mutex.h"


flconc::Coroutine::ptr cor;
flconc::Coroutine::ptr cor2;

class Test {

 public:
  flconc::CoroutineMutex m_coroutine_mutex;
  int a = 1;
};
Test test_;
...
void* thread1_func(void*) {
  ...
  flconc::Coroutine::Resume(cor.get());
...
int main(int argc, char* argv[]) {
  ...
  cor = std::make_shared<flconc::Coroutine>(stack_size, sp, fun1);

  char* sp2 = reinterpret_cast<char*>(malloc(stack_size));
  cor2 = std::make_shared<flconc::Coroutine>(stack_size, sp2, fun2);
