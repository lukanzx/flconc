...
namespace flconc {
    ...
    void *AsyncLogger::excute(void *arg) {
        ...
        while (1) {
            ...
            std::stringstream ss;
            ss << ptr->m_file_path << ptr->m_file_name << "_" << ptr->m_date << "_" << LogTypeToString(ptr->m_log_type)
               << "_" << ptr->m_no << ".log";
    ...
    void Exit(int code) {
        ...
        printf("It's sorry to said we start flconc server error, look up log file to get more deatils!\n");
        gRpcLogger->flush();
        pthread_join(gRpcLogger->getAsyncLogger()->m_thread, NULL);
