...
namespace flconc {
    ...
    static const int MSG_REQ_LEN = 20;

    FlconcPbCodeC::FlconcPbCodeC() {
    ...
    void FlconcPbCodeC::encode(TcpBuffer *buf, AbstractData *data) {
        if (!buf || !data) {
            ErrorLog << "encode error! buf or data nullptr";
            return;
        }

        FlconcPbStruct *tmp = dynamic_cast<FlconcPbStruct *>(data);
    ...
    const char *FlconcPbCodeC::encodePbData(FlconcPbStruct *data, int &len) {
    ...
    void FlconcPbCodeC::decode(TcpBuffer *buf, AbstractData *data) {
        ...
        DebugLog << "m_read_buffer size=" << buf->getBufferVector().size() << "rd=" << buf->readIndex() << "wd="
                 << buf->writeIndex();


        FlconcPbStruct *pb_struct = dynamic_cast<FlconcPbStruct *>(data);
        ...
        if (service_name_len_index >= end_index) {
            ErrorLog << "parse error, service_name_len_index[" << service_name_len_index << "] >= end_index["
                     << end_index << "]";
    ...
    ProtocalType FlconcPbCodeC::getProtocalType() {
        return FlconcPb_Protocal;
    }
