...
namespace flconc {
    ...
    extern flconc::Logger::ptr gRpcLogger;
    extern flconc::TcpServer::ptr gRpcServer;
    ...
    Config::Config(const char *file_path) : m_file_path(std::string(file_path)) {
        ...
        if (!rt) {
            printf("start flconc server error! read conf file [%s] error info: [%s], errorid: [%d], error_row_column:[%d row %d column]\n",
                   file_path, m_xml_file->ErrorDesc(), m_xml_file->ErrorId(), m_xml_file->ErrorRow(),
                   m_xml_file->ErrorCol());
            exit(0);
        }
    }
    ...
    void Config::readLogConfig(TiXmlElement *log_node) {
        ...
        if (!node || !node->GetText()) {
            printf("start flconc server error! read config file [%s] error, cannot read [log_path] xml node\n",
                   m_file_path.c_str());
            exit(0);
        }
        ...
        if (!node || !node->GetText()) {
            printf("start flconc server error! read config file [%s] error, cannot read [log_prefix] xml node\n",
                   m_file_path.c_str());
            exit(0);
        }
        ...
        if (!node || !node->GetText()) {
            printf("start flconc server error! read config file [%s] error, cannot read [log_max_file_size] xml node\n",
                   m_file_path.c_str());
            exit(0);
        }
        ...
        if (!node || !node->GetText()) {
            printf("start flconc server error! read config file [%s] error, cannot read [rpc_log_level] xml node\n",
                   m_file_path.c_str());
            exit(0);
        }
        ...
        if (!node || !node->GetText()) {
            printf("start flconc server error! read config file [%s] error, cannot read [app_log_level] xml node\n",
                   m_file_path.c_str());
            exit(0);
        }
        ...
        if (!node || !node->GetText()) {
            printf("start flconc server error! read config file [%s] error, cannot read [log_sync_inteval] xml node\n",
                   m_file_path.c_str());
            exit(0);
        }
    ...
    void Config::readDBConfig(TiXmlElement *node) {
        ...
        if (!node) {
          printf("start flconc server error! read config file [%s] error, cannot read [database] xml node\n", m_file_path.c_str());
          exit(0);
        }
    ...
    void Config::readConf() {
        ...
        if (!log_node) {
            printf("start flconc server error! read config file [%s] error, cannot read [log] xml node\n",
                   m_file_path.c_str());
            exit(0);
        }
        ...
        if (!time_wheel_node) {
            printf("start flconc server error! read config file [%s] error, cannot read [time_wheel] xml node\n",
                   m_file_path.c_str());
            exit(0);
        }
        ...
        if (!coroutine_node) {
            printf("start flconc server error! read config file [%s] error, cannot read [coroutine] xml node\n",
                   m_file_path.c_str());
            exit(0);
        }
        ...
        if (!coroutine_node->FirstChildElement("coroutine_stack_size") ||
            ...
            printf("start flconc server error! read config file [%s] error, cannot read [coroutine.coroutine_stack_size] xml node\n",
                   m_file_path.c_str());
            exit(0);
        }
        ...
        if (!coroutine_node->FirstChildElement("coroutine_pool_size") ||
            ...
            printf("start flconc server error! read config file [%s] error, cannot read [coroutine.coroutine_pool_size] xml node\n",
                   m_file_path.c_str());
            exit(0);
        }
        ...
        if (!root->FirstChildElement("msg_req_len") || !root->FirstChildElement("msg_req_len")->GetText()) {
            printf("start flconc server error! read config file [%s] error, cannot read [msg_req_len] xml node\n",
                   m_file_path.c_str());
            exit(0);
        }
        ...
        if (!root->FirstChildElement("max_connect_timeout") ||
            ...
            printf("start flconc server error! read config file [%s] error, cannot read [max_connect_timeout] xml node\n",
                   m_file_path.c_str());
            exit(0);
        }
        ...
        if (!root->FirstChildElement("iothread_num") || !root->FirstChildElement("iothread_num")->GetText()) {
            printf("start flconc server error! read config file [%s] error, cannot read [iothread_num] xml node\n",
                   m_file_path.c_str());
            exit(0);
        }
        ...
        if (!time_wheel_node->FirstChildElement("bucket_num") ||
            ...
            printf("start flconc server error! read config file [%s] error, cannot read [time_wheel.bucket_num] xml node\n",
                   m_file_path.c_str());
            exit(0);
        }
        if (!time_wheel_node->FirstChildElement("inteval") ||
            !time_wheel_node->FirstChildElement("inteval")->GetText()) {
            printf("start flconc server error! read config file [%s] error, cannot read [time_wheel.bucket_num] xml node\n",
                   m_file_path.c_str());
            exit(0);
        }
        ...
        if (!net_node) {
            printf("start flconc server error! read config file [%s] error, cannot read [server] xml node\n",
                   m_file_path.c_str());
            exit(0);
        }
        ...
        if (!net_node->FirstChildElement("ip") || !net_node->FirstChildElement("port") ||
            ...
            printf("start flconc server error! read config file [%s] error, cannot read [server.ip] or [server.port] or [server.protocal] xml node\n",
                   m_file_path.c_str());
            exit(0);
        }
        ...
        if (port == 0) {
            printf("start flconc server error! read config file [%s] error, read [server.port] = 0\n",
                   m_file_path.c_str());
            exit(0);
        }
        ...
        flconc::IPAddress::ptr addr = std::make_shared<flconc::IPAddress>(ip, port);

        if (protocal == "HTTP") {
            gRpcServer = std::make_shared<TcpServer>(addr, Http_Protocal);
        } else {
            gRpcServer = std::make_shared<TcpServer>(addr, FlconcPb_Protocal);
        }
