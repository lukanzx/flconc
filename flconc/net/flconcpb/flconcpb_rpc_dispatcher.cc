...
namespace flconc {
    ...
    void FlconcPbRpcDispacther::dispatch(AbstractData *data, TcpConnection *conn) {
        ...
        InfoLog << "begin to dispatch client flconcpb request, msgno=" << tmp->msg_req;

        std::string service_name;
        std::string method_name;

        FlconcPbStruct reply_pk;
        ...
        if (it == m_service_map.end() || !((*it).second)) {
            ...
            InfoLog << "end dispatch client flconcpb request, msgno=" << tmp->msg_req;
            return;
        ...
        FlconcPbRpcController rpc_controller;
        ...
        FlconcPbRpcClosure closure(reply_package_func);
    ...
    bool FlconcPbRpcDispacther::parseServiceFullName(const std::string &full_name, std::string &service_name,
                                                     std::string &method_name) {
        if (full_name.empty()) {
    ...
    void FlconcPbRpcDispacther::registerService(service_ptr service) {
