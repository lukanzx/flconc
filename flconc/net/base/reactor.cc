...
namespace flconc {
    ...
    void Reactor::loop() {
        ...
        while (!m_stop_flag) {
            ...
            if (first_coroutine) {
                flconc::Coroutine::Resume(first_coroutine);
            ...
            if (m_reactor_type != MainReactor) {
                ...
                while (1) {
                    ...
                    if (ptr) {
                        ptr->setReactor(this);
                        flconc::Coroutine::Resume(ptr->getCoroutine());
                    } else {
                        break;
                    }
                }
            ...
            if (rt < 0) {
                ...
                for (int i = 0; i < rt; ++i) {
                    ...
                    if (one_event.data.fd == m_wake_fd && (one_event.events & READ)) {
                        ...
                        flconc::FdEvent *ptr = (flconc::FdEvent *) one_event.data.ptr;
                        if (ptr != nullptr) {
                            int fd = ptr->getFd();
    ...
    void Reactor::addCoroutine(flconc::Coroutine::ptr cor, bool is_wakeup /*=true*/) {
