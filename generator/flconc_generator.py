...
class Generator:
    ...
    def gen_exception(self):
        ...
        if not os.path.exists(exception_file):
            ...
            exception_template = Template(
                open(self.generator_path + '/template/business_exception.h.template', 'r').read())
    ...
    def gen_each_interface(self):
        ...
        interface_test_client_file_template = Template(
            open(generator_path + '/template/test_flconc_client.cc.template', 'r').read())
        ...
        for each in self.interface_list:
            ...
            if not os.path.exists(file):
                cc_file_content = interface_cc_file_temlate.safe_substitute(
                    PROJECT_NAME=project_name,
                    INCLUDE_PB_HEADER=self.INCLUDE_PB_HEADER,
                    INCLUDE_INTERFACE_HEADER='#include "' + project_name + '/interface/' + each[
                        'interface_name'] + '.h"',
                    CREATE_TIME=datetime.now().strftime('%Y-%m-%d %H:%M:%S'),
                    CLASS_NAME=each['interface_class_name'],
                    REQUEST_TYPE=each['request_type'],
        ...
        print('End generate flconc framework code')
        print('=' * 100)

    def generate_framework_code(self):
        print('=' * 100)
        print('Begin to generate flconc framework code')
    ...
    def gen_conf_file(self):
        print('=' * 100)
        print('Begin to generate flconc conf file')
        ...
        file.close()
        print('succ write to ' + out_file)
        print('End generate flconc conf file')
        print('=' * 100)
    ...
    def generate_flconc_project(self):
        try:

            self.parseInput()

            print('=' * 150)
            print('Begin generate flconc project')
            ...
            print('Succ generate flconc project')
            print('=' * 150)

        except Exception as e:
            print('Falied generate flconc project, err:')
            print(e)
            traceback.print_exc()
            print('=' * 150)
        finally:
    ...
    def printHelp(self):
        ...
        print('Usage:')
        print('flconc_generator.py -[options][target]\n')
        print('Options:')
        print('-h, --help')
        ...
        print('')
        print('For example:')
        print('flconc_generator.py -i order_server.proto -o ./')
...
if __name__ == '__main__':
    instanse = Generator()
    instanse.generate_flconc_project()
